...
// of the pipeline:
//
//   1. `embedSwiftExportForXcode` produced `TracingTest.swiftmodule/`
...
//      `import TracingTest` above would fail at compile time.
//
//   2. The static archive `libTracingTest.a` (produced by the
...
//      architecture arm64".
//
//   3. The Kotlin `swiftExport { moduleName = "TracingTest" }` and
//      `flattenPackage = "io.github.kotlinmania.tracingtest"` configuration in
...
//      via the `TracingTestLibrary` product.
//
...
final class TracingTestExportTests: XCTestCase {
    func testSwiftModuleLoads() throws {
        XCTAssertTrue(true, "TracingTest swift module imported cleanly")
    }
}
