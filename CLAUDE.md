# Claude Code Project Instructions — tracing-test-kotlin

## Project Overview

This is **tracing-test-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`tracing-test`](https://crates.io/crates/tracing-test).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/tracing-test/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/tracing-test>
