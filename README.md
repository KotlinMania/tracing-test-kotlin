...
Kotlin Multiplatform line-by-line clean-room port of the Rust crate [`tracing-test`](https://crates.io/crates/tracing-test).

> **Status: scaffold — porting has not started.** This repo currently contains build infrastructure only. The upstream Rust source for the `tracing-test` crate will be cloned into `tmp/` (gitignored) when porting begins.

## About

Test helpers for `tracing`
...
```kotlin
dependencies {
    implementation("io.github.kotlinmania:tracing-test-kotlin:0.1.0-SNAPSHOT")
}
```
...
Original work copyrighted by the upstream `tracing-test` authors. Kotlin port copyright (c) 2026 Sydney Renee and The Solace Project.
